...
CatBoost cao hơn về PR-AUC/ROC-AUC (thước đo không phụ thuộc ngưỡng). Ở ngưỡng mặc định 0.5
...
Dự án có **2 phiên bản dashboard** để demo:
...
- **Top rủi ro cao nhất**: chấm điểm toàn bộ nhân sự hiện có, xếp hạng Top N kèm lý do chính (SHAP) — sẵn sàng dùng để triage ngay, không cần chuẩn bị dữ liệu gì thêm

### Deploy Gradio lên Hugging Face Spaces 
...
Không phải mọi thứ đúng ngay từ lần chạy đầu — 3 lỗi dưới đây bắt được khi số liệu
